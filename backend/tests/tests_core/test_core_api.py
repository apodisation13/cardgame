...
class TestModels:
    ...
    def test_api_admin_actions_json(self, create_admin):
        """Проверка эндпоинта для админа"""
        admin = create_admin()
        self.api_client.force_authenticate(admin)
        response_admin = self.api_client.get("/api/v1/game_prices/")
        assert response_admin.status_code == HTTP_200_OK

    def test_api_user_actions_json(self, create_user):
        """Проверка эндпоинта для юсера"""
        user = create_user()
        self.api_client.force_authenticate(user)
        response_user = self.api_client.get("/api/v1/game_prices/")
        assert response_user.status_code == HTTP_200_OK

    def test_api_methode_actions_json(self, create_admin):
        """Проверка запрета на методы POST, DELETE"""
        admin = create_admin()
        self.api_client.force_authenticate(admin)
        response_post = self.api_client.post("/api/v1/game_prices/")
        response_del = self.api_client.delete("/api/v1/game_prices/")
